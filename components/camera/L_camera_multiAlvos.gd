...
@export_range(0,10000,50) var aumentoDoZoom = 900
...
func _process(delta: float) -> void:
	...
	var zoom = clamp(aumentoDoZoom/mais_distante, zoomLonge,zoomProximo)
	
	camera.zoom = Vector2(zoom, zoom)
	camera.global_position = lerp(camera.global_position, pontoMedio, delta * suavidade)
